...
namespace MyForce.RadioModules.MotorolaXpr;

/// <summary>
/// Discovery entry point for the Motorola XPR radio module. The Audio Processor loads
/// this assembly from its plugins folder and instantiates this factory by reflection.
/// </summary>
public sealed class MotorolaXprModuleFactory : IRadioModuleFactory
{
	public string TypeId => "motorola_xpr";

	public string DisplayName => "Motorola XPR";
	...
	// sections via RadioModuleSchemaBuilder. Add XPR-specific settings here as they exist.
	...
	// Motorola XPR uses AP relay keying and AP VOX detection, and does not provide its own audio.
	...
	public IRadioModule Create(IModuleHost host) => new MotorolaXprModule(host);
}

/// <summary>
/// Runtime behavior for a single Motorola XPR instance. This is a skeleton: it loads and
/// reports cleanly so the AP can discover and host it, but performs no hardware control yet.
/// </summary>
public sealed class MotorolaXprModule : IRadioModule
{
	...
	{
		_host.Log(LogLevel.Info, "Motorola XPR module started (skeleton; no hardware control implemented yet).");
